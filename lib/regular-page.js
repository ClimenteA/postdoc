...
import PandocCompiler from './pandoc-compiler.js';
...
export default class RegularPage extends Page {
  ...
  constructor(contentFilePath, temporaryOutputDirectoryName, markdownCompiler) {
    ...
    this.#pandoc = new PandocCompiler();
  }

  async #compileContent(allowDrafts) {

    if (this.#contentFilePath.endsWith(".rst") || this.#contentFilePath.endsWith(".ipynb")) {
      this.#contentFilePath = this.#pandoc.compileToMarkdown(this.#contentFilePath);
    }

    console.log("heloooooooo", this.#contentFilePath)
